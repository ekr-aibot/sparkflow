...
**Workflow listing:** Concatenates `.sparkflow/workflows/` (project) and `~/.sparkflow/flows/` (user) with project shadowing user on name conflicts. Aborts with an error if both are empty. The default-workflow select only shows workflows with `"kind": "main"`; the monitors checkbox only shows workflows with `"kind": "monitor"`. Workflows with `"kind": "helper"` or no kind are hidden from both prompts. Each choice displays the workflow's `description` field (if present, truncated to 200 chars) as inline help via inquirer's `description` property. Choice names are plain workflow names (no `(user)` / `(project)` suffix). The monitors checkbox is skipped entirely when no `kind: "monitor"` workflows exist.
